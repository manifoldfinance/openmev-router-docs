...
contract KashiFlash is IFlashLoanReceiver {
    ...
    **/
    function liquidateWithFlashloan(
        ...
        uint256 borrowAmount = Kashi(kashiAddr).userBorrowPart(target);
        Rebase memory totalBorrow = Kashi(kashiAddr).totalBorrow();
        uint256 ratio = (totalBorrow.elastic + 1) / totalBorrow.base;
        // Get USDC flashloan
        liq = Liquidation({
            target: target,
    ...
    function executeOperation(
        ...
        bentobox.deposit(IERC20(assets[0]), address(this), address(this), amounts[0], 0);
        ...
        uint256 collateralRetrieved;
        {
            ...
            uint256[] memory amountsMinusFees = new uint256[](1);
        ...
        // Swap collateral
        {
            uint256 loanPlusPremium = amounts[0] + premiums[0];
            ...
            uint256 bal = IERC20(liq.collateral).balanceOf(address(this));
            ...
            uint256 borrowBalance = IERC20(assets[0]).balanceOf(address(this));
    ...
    function liquidate(
        ...
        uint256 collateralBalanceBefore = bentobox.balanceOf(collateral, address(this));
        uint256 borrowBalanceBefore = bentobox.balanceOf(borrow, address(this));

        bentobox.setMasterContractApproval(address(this), Kashi(kashiAddr).masterContract(), true, 0, 0x0, 0x0);
        ...
        Kashi(kashiAddr).liquidate(users, maxBorrowParts, address(this), ISwapper(address(0x0)), true);

        uint256 collateralBalanceAfter = bentobox.balanceOf(collateral, address(this));
        uint256 borrowBalanceAfter = bentobox.balanceOf(borrow, address(this));
        require(
            collateralBalanceAfter > collateralBalanceBefore,
            "BentoBox balance did not increase after liquidation"
        );
    }
}
